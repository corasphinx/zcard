...
class FacebookEmbedScreen extends Component {
        ...
        (Zmodule_Info) => {
          ...
          if (Zmodule_Info) {
            ...
            this.props.callClassFunction(
              ...
              (Zmodule_Data) => {
                if (Zmodule_Data) {
                  this.setState({ facebook_url: Zmodule_Data[0].facebook_url })
                }
              },
            );
          }
  ...
  isValidate = () => {
    const { section_title, facebook_url } = this.state;
    if (section_title == '') return 'You must supply a Zmodule Title to save!';
    if (facebook_url == '') return 'You must supply a URL to save!';
    return '';
  }
  ...
  saveSection = (identifier, zcard, section, page) => {
    const { section_title, facebook_url, tab_color, tab_font_color } = this.state;
    // save title
    this.props.callController(
